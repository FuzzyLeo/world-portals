...
7. On first-time-predicted client OR on server: also call `ply:SetPos(newPos)`. `ply:SetPos` is the canonical entity-snap path (same one the broadcast hits for remote clients) and resets the AbsOrigin interp cache that `mv:SetOrigin` doesn't touch. Skipped during resimulation (would re-snap to the same value).
8. Call `ply:SetEyeAngles(clampedAng)` on **both realms**, but gated differently: on the client only when `IsFirstTimePredicted()`, on the server unconditionally. `ply:SetEyeAngles` writes the persistent `m_angEyeAngles` field. *Client, first-time only:* it's what actually rotates the camera (`cmd:SetViewAngles` alone leaves the camera on its last-input value — directional portals no-op without it); gating on first-time keeps **client resim** from clobbering mouse delta accumulated since (camera "snaps back" mid-teleport). *Server:* required to stop the engine's prediction-error correction from rolling the predicted angle back. The cmd the server received carries the user's *pre-teleport* viewangles (the client's `cmd:SetViewAngles` doesn't propagate to the network-serialized copy), so without an explicit server write the snapshot reflects the pre-teleport angle and ~RTT later the client's correction snaps from `clampedAng` back to it — the "angle changes without moving the mouse" bug. Writing the same value on both realms makes the snapshot match the prediction so the correction never fires. (An earlier version of this file claimed the server must *not* set it; that was wrong and caused the rollback — don't revert to client-only.)
9. Server branch: VR yaw offset (`WorldPortals_VRMod_SetAngle`), `ForcePlayerDrop`, entity outputs, `wp-teleport` hook, broadcast `WorldPortals_Teleport`.
10. First-time-predicted client branch: roll fade trigger (`wp.rotating = newAng.r` for `cl_teleport.lua`'s `CalcView` to interpolate down), predict-lerp arm (`wp.predictedPos = newPos`, `wp.predictedAt = CurTime()` — see below), `wp-teleport` hook, debug-HUD record.
