...
- Convars `worldportals_ghosts` (master) / `worldportals_show_self` (own body; created in `cl_render.lua`, also gates `ShouldDrawLocalPlayer` so off = no self-reflection in RTs). Consumer hooks `wp-shouldghost`/`wp-shouldghostdraw` under Conventions.
...
- **`wp-nocollide(portal, ent)`** (server) — return a list of extra solids to pass through, on top of parent + constraint network. Only no-collided if it also sets `ent.PortalNoCollide == true` — default-solid is deliberate, so a missed flag merely *jams* a prop (recoverable) instead of dropping it into the void. Field contracts: `ent.PortalNoCollide == true` = a solid the pass-through may phase; `ent.WPIsGhost == true` = a clientside ghost (collision + ghost passes skip it). Don't repurpose either.
